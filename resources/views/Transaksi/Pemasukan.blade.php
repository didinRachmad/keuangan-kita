...
@section('content')
                                        ...
                                        <td class="text-center">
                                            @if (Gate::allows('role-admin') || $data->id_user == Auth::user()->id)
                                                <a href="javascript:void(0)"
                                                    class="btn btn-warning shadow-warning mx-2 btnEditPemasukan"
                                                    data-id="{{ $data->id }}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" id="formPemasukan" action="">
                        ...
                        <div class="mb-3">
                            <label for="total_transaksi" class="form-label">Total Pemasukan (Rp)</label>
    ...
    <script>
        $(document).ready(function() {
            $('#modalPemasukan').on('shown.bs.modal', function() {
                // SELECT HARI AWAL
                $('#akun').select2({
                    theme: 'bootstrap-5',
                    ajax: {
                        url: "{{ route('Pemasukan.getAkun') }}",
                        dataType: 'json',
                        // delay: 250,
                ...
                $('#kategori').select2({
                    theme: 'bootstrap-5',
                    ajax: {
                        url: "{{ route('Pemasukan.getKategori') }}",
                        dataType: 'json',
                        // delay: 250,
                        data: function(params) {
                            return {
            ...
            $('.btnEditPemasukan').click(function() {
                ...
                $('#modalPemasukan').find('#tgl_transaksi').val(tgl_transaksi);
                var optionKategori = $('<option></option>')
                    .attr('value', id_kategori)
                    .attr('selected', 'selected')
                    .text(kategori);
                $('#modalPemasukan').find('#kategori').append(optionKategori);
                var optionAkun = $('<option></option>')
                    .attr('value', id_akun)
                    .attr('selected', 'selected')
                    .text(akun);
                $('#modalPemasukan').find('#akun').append(optionAkun);
                var optionPJ = $('<option></option>')
                    .attr('value', id_pj)
                    .attr('selected', 'selected')
                    .text(pj);
                $('#modalPemasukan').find('#pj').append(optionPJ);
